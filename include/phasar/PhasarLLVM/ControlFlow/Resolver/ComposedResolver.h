...
template <typename Res1T, typename Res2T> struct ComposedResolver {
  ...
  mutatesHelperAnalysisInformation() const noexcept {
    return LLVMResolverTraits::mutatesHelperAnalysisInformation(Res1) ||
           LLVMResolverTraits::mutatesHelperAnalysisInformation(Res2);
  }
  ...
  handlePossibleTargets(const llvm::CallBase *CallSite,
    ...
    LLVMResolverTraits::handlePossibleTargets(Res1, CallSite, CalleeTargets);
    LLVMResolverTraits::handlePossibleTargets(Res2, CallSite, CalleeTargets);
  }
};
...
#else
    std::enable_if_t<LLVMResolverTraits::IResolver<R1> &&
                         LLVMResolverTraits::IResolver<R2>,
