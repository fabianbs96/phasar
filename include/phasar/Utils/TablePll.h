...
template <typename R, typename C, typename V> class TablePll {
public:
  using Cell = TableCell<R, C, V>;

  TablePll() noexcept = default;

  explicit TablePll(const TablePll &T) = default;
  TablePll &operator=(const TablePll &T) = delete;

  TablePll(TablePll &&T) noexcept = default;
  TablePll &operator=(TablePll &&T) noexcept = default;
  ...
  bool operator==(const TablePll<R, C, V> &Other) noexcept {
