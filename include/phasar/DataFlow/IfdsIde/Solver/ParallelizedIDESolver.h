...
#ifndef PHASAR_DATAFLOW_IFDSIDE_SOLVER_PARALLELIZED_IDESOLVER_H
#define PHASAR_DATAFLOW_IFDSIDE_SOLVER_PARALLELIZED_IDESOLVER_H
...
class ParallelizedIDESolver
  ...
  friend IDESolverAPIMixin<ParallelizedIDESolver<AnalysisDomainTy, Container>>;
...
public:
  ...
  ParallelizedIDESolver(ParallelizedIDESolver &&) = delete;
  ParallelizedIDESolver &operator=(ParallelizedIDESolver &&) = delete;
  ...
  resultsAt(n_t Stmt, bool StripZero = false) /*TODO const*/ {
    auto Results = getSolverResults().resultsAt(Stmt, StripZero);
    std::unordered_map<d_t, l_t> ReturnResults(Results.size());

    for (auto Entry : Results) {
      ReturnResults.insert(std::move(Entry));
    }
  ...
  ///
  /// NOTE: The SolverResults store a reference into this ParallelizedIDESolver,
...
protected:
      ...
      if (SpecialSum) {
        ...
        for (n_t SP : StartPointsOf) {
          ...
          for (d_t d3 : Res) {
            ...
            // edge into <sP,d3>
            for (const CellOfTable &Entry : endSummary(SP, d3)) {
  ...
  virtual void saveEdges(n_t SourceNode, n_t SinkStmt, d_t SourceVal,
    ...
    TablePll<n_t, n_t, phmap::parallel_node_hash_map<d_t, container_type>>
  ...
  virtual void processExit(const PathEdge<n_t, d_t> Edge) {
    ...
    phmap::parallel_node_hash_map<n_t, container_type> Inc;
    for (n_t SP : StartPointsOf) {
  ...
  /// implementation but may be useful for subclasses of ParallelizedIDESolver)
  ...
  /// but may be useful for subclasses of {@link ParallelizedIDESolver})
  ///
...
private:
  ...
  TablePll<n_t, d_t, TablePll<n_t, d_t, EdgeFunction<l_t>>> EndsummaryTab;

  // edges going along calls
  // see CC 2010 paper by Naeem, Lhotak and Rodriguez
  TablePll<n_t, d_t, phmap::parallel_node_hash_map<n_t, Container>> IncomingTab;
...
operator<<(llvm::raw_ostream &OS,
           const ParallelizedIDESolver<AnalysisDomainTy, Container> &Solver) {
  Solver.dumpResults(OS);
  return OS;
}

template <typename Problem, typename ICF>
ParallelizedIDESolver(Problem &, ICF *)
    -> ParallelizedIDESolver<typename Problem::ProblemAnalysisDomain,
